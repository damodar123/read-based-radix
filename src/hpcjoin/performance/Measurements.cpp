...
void Measurements::init(uint32_t nodeId, uint32_t numberOfNodes, std::string tag) {
	...
	if (nodeId == hpcjoin::core::Configuration::RESULT_AGGREGATION_NODE) 
	{
		rcv_string = (char *)malloc(MPI_MAX_PROCESSOR_NAME*numberOfNodes);
	}
	MPI_Get_processor_name(processor_name, &name_len);
	MPI_Gather(processor_name, MPI_MAX_PROCESSOR_NAME, MPI_CHAR,
			rcv_string, MPI_MAX_PROCESSOR_NAME, MPI_CHAR, hpcjoin::core::Configuration::RESULT_AGGREGATION_NODE, MPI_COMM_WORLD );
	...
	if (nodeId == hpcjoin::core::Configuration::RESULT_AGGREGATION_NODE) 
	{
		...
		{
			printf("%s ", &rcv_string[i*MPI_MAX_PROCESSOR_NAME]);
		}
		printf("\n");
		free(rcv_string);
	}
