...
uint64_t read_cmdline_args(int argc, char *argv[])
{
	uint64_t tuples_relation_core;
	for (int i = 0; i < argc; ++i)
	{
		switch(i)
		{
			case 1:
				tuples_relation_core = atoi(argv[i]);
			break;
		}
	}
	return tuples_relation_core;
}
...
int main(int argc, char *argv[]) {
	...
	uint64_t tuples_per_relation_per_core = 0;
	tuples_per_relation_per_core = read_cmdline_args(argc, argv);
	if (tuples_per_relation_per_core == 0) {
		printf("Error: Number of Tuples cannot be 0");
		return 0;
	}
	uint64_t globalInnerRelationSize = ((uint64_t) numberOfNodes) * tuples_per_relation_per_core;
	uint64_t globalOuterRelationSize = ((uint64_t) numberOfNodes) * tuples_per_relation_per_core;
	...
	if (nodeId != hpcjoin::core::Configuration::RESULT_AGGREGATION_NODE) {
		...
		printf("numberoftuples = %d, Partition count: Network=%d, Local=%d\n", tuples_per_relation_per_core,
